...
describe User do
  ...
  describe "Managers" do
    ...
    #Egregiously c/p from Hartl Ch. 9
    it "should be a non-manager by default" do
      @user = User.create!(@attr)
      @user.should_not be_manager
    end
   
    it "should be manager if set to manager" do
      @user = User.create!(@attr)
      @user.toggle!(:manager)
      @user.should be_manager
    end
   
    it "manager? should return true if manager" do
      @user = User.create!(@attr)
      @user.toggle!(:manager)
      @user.manager?.should be_true
    end 

    it "manager? should return false if not manager" do
      @user = User.create!(@attr)
  ...
  it "should accept valid email addresses" do
    addresses = %w[user@foo.com THE_MANAGER@foo.bar.org first.last@foo.jp]
